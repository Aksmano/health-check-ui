...
import {
  fetchCitiesRequest,
  fetchCitiesSuccess,
  loadMedicalOffersRequest,
  loadMedicalOffersSuccess,
  MedicalOfferActionType,
} from '../actions/medical-offer.actions';
...
export class MedicalOfferEffects {
  ...
  public readonly fetchCities$ = createEffect(() =>
    this.actions$.pipe(
  ...
  public readonly loadOffersForAddress$ = createEffect(() =>
    this.actions$.pipe(
      ofType(loadMedicalOffersRequest),
      switchMap((action) =>
        this.medicalOfferService.getMedicalOffers(action.city, action.address)
      ),
      map((offerItems) =>
        loadMedicalOffersSuccess({ medicalOfferItems: offerItems })
      ),
      catchError((err, caught) => {
