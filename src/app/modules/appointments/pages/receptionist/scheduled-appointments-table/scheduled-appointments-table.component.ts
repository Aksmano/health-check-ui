...
export class ScheduledAppointmentsTableComponent {
  ...
  protected weekEnd: Date = new Date(
    this.getWeekBeginning().getTime() + ONE_DAY_IN_MILISECONDS * 6
  );
  ...
  private getAppointments() {
    if (!!this.departmentId) {
      ...
      this.appointmentService
        .getAppointmentsByDepartmentId(this.departmentId, {
          startDateTime: this.weekBeginning,
          endDateTime: new Date(
            this.weekBeginning.getTime() + ONE_WEEK_IN_MILLISECONDS
          ),
        })
        .subscribe({
          next: (data) => {
          ...
          error: (error) => {
            this.toastService.showError(
              'Error during fetching tests for department.'
            );
          },
  ...
  incrementCurrentDate() {
    this.weekBeginning = new Date(
      this.weekBeginning.getTime() + ONE_WEEK_IN_MILLISECONDS
    );
    this.weekEnd = new Date(
      this.weekBeginning.getTime() + ONE_DAY_IN_MILISECONDS * 6
    );
    this.getAppointments();
  }
  ...
  deleteAppointment(id: number) {
    this.isProcessOngoing = true;
    this.appointmentService.removeAppointmentById(id).subscribe({
      next: (res) => {
      ...
      error: (error) => {
        this.toastService.showError(
          'Something went wrong while downloading appointment, try again later.'
        );
