...
export class AddAppointmentSchedulesComponent {
  ...
  ngOnInit(): void {
    this.activatedRoute.params.subscribe(params => {
      ...
      this.dateSubscription = this.dateSubscription = this.currentDate$.subscribe(data => {
        this.currentDate = data;
        if (this.schedulesSubscription) {
          this.schedulesSubscription.unsubscribe();
        }

        this.doctorService.getAllDoctors({ departmentId: this.departmentId, specialization: this.spec })
          .subscribe(doctors => {
  ...
  onDoctorSelect() {
    if (!!this.selectedDoctor)
      this.schedulesSubscription = this.scheduleService.getSchedules(this.selectedDoctor.code, {
        startDateTime: getFirstDayOfWeek(this.currentDate!),
        endDateTime: getTheLastDayOfWeek(this.currentDate!)
      } as SchedulesAppointmentsCriteriaQP).subscribe({
        next: data => {
  ...
  isDateInSchedule(hour: Date, day: Date): boolean {
    for (let i = 0; i < this.assignedSchedules!.length; i++) {
      ...
      if (schedule.startDateTime.getFullYear() == day.getFullYear() && schedule.startDateTime.getMonth() == day.getMonth()
        && schedule.startDateTime.getDate() == day.getDate()) {
        if (schedule.startDateTime.getHours() <= hour.getHours() && hour.getHours() < schedule.endDateTime.getHours()) {
          return true;
        }
      }
    }
  ...
  showTimeInsertWindow(day: Date, hour: Date) {
    this.chosenDate = new Date(day.getFullYear(), day.getMonth(), day.getDate(), hour.getHours())
  ...
  addSchedule() {
    ...
    this.scheduleService.addSchedulesByDoctorId(this.selectedDoctor?.code!, rq).subscribe(
      {
        next: data => {
